			...
			<div className="py-14  relative overflow-hidden justify-center">
				<div className=" container px-5 py-4 mx-auto">
					...
					<div className="flex flex-wrap -m-4 lg:justify-center xl:justify-start" data-aos='fade-up' data-aos-delay='200'>
						...
						{
							skills.map((item,i ) => (
								<div key={i} className={`${color}  p-4 lg:w-1/4 md:w-1/2 w-full min-w-70`} data-aos='fade-up' data-aos-delay={`${100 + i * 150}`}>
									<div  className=" h-full p-4 rounded-2xl border border-gray-600 hover:border-green-700 hover:border-2 transition-all duration-300  hover:-translate-y-2 group hover:shadow-[0_0_30px_rgb(255, 165, 0, 0.15)]">
											<div className="flex items-center m-2">
												<div className={`bg-linear-to-r  from-gray-300 to-gray-100 w-16 h-16 rounded-xl p-3 flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
													<img src={item.icon} alt={item.name} className="w-full h-full object-contain"/>
												</div>
												<h3 className={`text-xl font-bold ml-4 ${color} `}>{item.name} </h3>
											</div>
									</div>
								</div>
							))
